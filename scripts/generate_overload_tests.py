"""
Generate Python typecheck test files for ABI type string combinations.

Usage:
    python scripts/generate_overload_tests.py
        [--impl codec|abi|both]
        [--suite fixed|var|both]
        [--sample-1-of-x N]
        [--seed S]
        [--lengths L1,L2,...]
...
- tests/typecheck/abi/decode/fixed/len_1/overload_test_data_*.py
  (tuple-based, abi.decode)
- tests/typecheck/abi/decode/variable/len_1/overload_test_data_*.py
  (iterable-based, abi.decode)
...
...
- For TupleXXXIntTypeStr, only 10 representative values are sampled.
- Each chunk's lines are collected and written in a single operation for efficiency.
- Covers int, uint, bytes, bool, string, arrays, and combinations up to
  length 3, including fallback/unknown types in all permutations.
...
- --sample-1-of-x N: Only generate a random 1/N of the chunk files.
- --seed S: Seed for reproducible random sampling (default: 42).
- --lengths L1,L2,...: Only generate data for the specified lengths.
"""
...
from typing import (
    Any as {ANY_ALIAS},
    Tuple as {TUPLE_ALIAS},
    Union as {UNION_ALIAS},
)

from eth_typing import (
    HexAddress as {HEXADDRESS_ALIAS},
)
from typing_extensions import (
    assert_type as {ASSERT_TYPE_ALIAS},
)
...
def extract_all_literals(typ, alias_map, alias_path=None):
    ...
    if origin is Union:
        ...
        if alias_path[-1] in ("BytesArrayTypeStr", "IntArrayTypeStr"):
            # TODO: implement overloads for these. For now only literals exist.
            ...
            for value in get_args(typ):
                if all_integers_in_set(value, {8, 16, 64, 128, 256}):
                    if value not in RETURN_TYPE_MAP:
                        ...
                        RETURN_TYPE_MAP[value] = (
                            f"{TUPLE_ALIAS}["
                            f"{', '.join(RETURN_TYPE_MAP[t] for t in types)}"
                            "]"
                        )
                    yield value
            return
        ...
        if ref_name in alias_map:
            ...
            return
        raise RuntimeError(
            f"ForwardRef {ref_name!r} not found in alias map "
            f"(path: {' -> '.join(alias_path)})"
        )
    else:
        raise RuntimeError(
            f"Type {typ!r} is not a Union, Literal, ForwardRef, or str "
            f"(origin={origin!r}, path: {' -> '.join(alias_path)})"
        )
...
def format_sequence_literal(case: Tuple[str, ...], opener: str, closer: str) -> str:
    values = ", ".join(format_string_literal(value) for value in case)
    if opener == "(" and len(case) == 1:
        values += ","
    return f"{opener}{values}{closer}"
    ...
    for L in lengths:
        ...
        with tqdm(
            ...
            def format_case(typestr: str, expected: str) -> str:
                ...
                comment = f"# {mode} case {case_counter}"
                one_line = (
                    f"{ASSERT_TYPE_ALIAS}({FUNC_ALIAS}({typestr}, {DATA_ALIAS}), "
                    f"{expected})  {comment}"
                )
                case_counter += 1
                file_counter += 1
                ...
                return (
                    f"{ASSERT_TYPE_ALIAS}(\n"
                    f"    {FUNC_ALIAS}({typestr}, {DATA_ALIAS}),\n"
                    f"    {expected},\n"
                    f")  {comment}\n"
                )
            ...
            if mode == "fixed":
                ...
                def addline(
                    case: Tuple[str, ...],
                    chunk_lines: List[str] = chunk_lines,
                ) -> None:
                    typestr = format_sequence_literal(case, "(", ")")
            ...
            def write_file(
                chunk_indices: List[int] = chunk_indices,
                path_without_number: str = PATH_WITHOUT_NUMBER,
                chunk_lines: List[str] = chunk_lines,
            ) -> None:
                nonlocal chunk_idx
...
def main():
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Only generate a random 1/N of the chunk files for each length "
            "(default: 1, i.e., all files)"
        ),
    )
    parser.add_argument(
        "--seed",
        ...
        default=42,
        help=(
            "Seed for random sampling (default: 42). Controls which chunk "
            "files are selected when sampling, for reproducibility."
        ),
    )
    parser.add_argument(
        "--lengths",
    ...
    for impl in impls:
        for mode in modes:
            suite_kind = "tuple" if mode == "fixed" else "iterable"
            print(f"Streaming {suite_kind}-based test cases for {impl}...")
