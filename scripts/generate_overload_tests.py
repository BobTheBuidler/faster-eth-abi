...
def stream_cases_and_write_files(
    ...
    max_chunks_per_length: Optional[int] = None,
    max_chunks_for_final_length: Optional[int] = None,
) -> int:
    ...
    for L in lengths:
        ...
        chunk_limit = (
            max_chunks_for_final_length
            if L == lengths[-1] and max_chunks_for_final_length is not None
            else max_chunks_per_length
        )
        ...
        if chunk_limit is not None:
            chunk_indices = [idx for idx in chunk_indices if idx <= chunk_limit]
        ...
        with tqdm(
            ...
            def write_file() -> None:
                nonlocal chunk_idx, stop_generation
...
def main():
    ...
    parser.add_argument(
        "--max-chunks-per-length",
        type=int,
        help="Only write the first N chunk files for each generated length.",
    )
    parser.add_argument(
        "--max-chunks-for-final-length",
        type=int,
        help="Override --max-chunks-per-length for the last requested length.",
    )
    ...
    for impl in impls:
        for mode in modes:
            ...
            stream_cases_and_write_files(
                ...
                seed=args.seed,
                lengths=lengths,
                max_chunks_per_length=args.max_chunks_per_length,
                max_chunks_for_final_length=args.max_chunks_for_final_length,
            )
