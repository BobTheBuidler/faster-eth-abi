...
import re
import sys
from typing import ForwardRef, List, Literal, Set, Tuple, Union, get_args, get_origin
...
MAX_LEN_VARIABLE = 4
CHUNK_SIZE = 10_000
FILENAME_PREFIX = "overload_test_data_"
...
def stream_cases_and_write_files(
    ...
    for L in range(1, MAX_LEN + 1):
        ...
        callee = "abi.decode" if impl == "abi" else "decoder.decode"
        ...
        for combo in itertools.product(all_literals, repeat=L):
            if file_counter and file_counter % CHUNK_SIZE == 0:
                ...
                write_file(path, chunk_lines, impl)
                chunk_lines.clear()
                chunk_idx += 1
                if progress:
                    progress.update(1)
...
def write_file(path: Path, lines: List[str], impl: Literal["abi", "codec"]) -> None:
