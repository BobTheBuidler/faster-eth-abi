...
PyInit_validation(void)
{
    ...
    tmp = PyImport_ImportModuleLevel("faster_eth_abi__mypyc", NULL, NULL, fromlist, 0);
    if (!tmp) return NULL;
    ...
    if (deps_capsule != NULL) {
        int (*deps_func)(void) = (int (*)(void))PyCapsule_GetPointer(
            deps_capsule, "faster_eth_abi__mypyc.ensure_deps");
    ...
    void *init_func = PyCapsule_GetPointer(capsule, "faster_eth_abi__mypyc.init_faster_eth_abi___utils___validation");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
...
PyMODINIT_FUNC PyInit___init__(void) { return PyInit_validation(); }
